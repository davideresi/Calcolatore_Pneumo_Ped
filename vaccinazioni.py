...
# --- Funzione che gestisce le dosi di PCV13 ---
def gestisci_pcv13(data_nascita, tipo_dosi, categoria, eta_dosi):
    data_limite_pcv13 = datetime(2024, 1, 1).date()
    if data_nascita < data_limite_pcv13 and "PCV13" in tipo_dosi:
        if "PCV20" in tipo_dosi:
            # Prende le età delle dosi di PCV20
            eta_pcv20 = [eta_dosi[i] for i, v in enumerate(tipo_dosi) if v == "PCV20"]
            if any(e >= 12 for e in eta_pcv20):
                st.warning("⚠️ Dose/i di PCV13 rilevata/e (vaccino utilizzato nei nati prima del 2024).")
                st.info("ℹ️ La/le dose/i di PCV13 non modifica/no la schedula se già presente almeno una dose di PCV20 dopo i 12 mesi.")
                if categoria == "Con patologia a rischio":
                    st.info("➕ Dopo i 24 mesi, raccomandata una dose di PPSV23 a distanza di almeno 8 settimane dall'ultima dose di PCV20.")
                return True
            else:
                st.warning("⚠️ Dose/i di PCV13 rilevata/e (vaccino utilizzato nei nati prima del 2024).")
                st.info("💡 Tutte le dosi di PCV20 sono state somministrate prima dei 12 mesi. Si raccomanda una dose di PCV20 dopo i 12 mesi.")
                if categoria == "Con patologia a rischio":
                    st.info("➕ Dopo i 24 mesi, raccomandata una dose di PPSV23 a distanza di almeno 8 settimane dall'ultima dose di PCV20.")
                return True
        else:
            st.warning("⚠️ Dose/i di PCV13 rilevata/e (vaccino utilizzato nei nati prima del 2024).")
            if categoria == "Con patologia a rischio":
                st.info("👉 Secondo le raccomandazioni attuali, la vaccinazione va completata con almeno 1 dose di PCV20.")
                st.info("➕ Dopo i 24 mesi, raccomandata una dose di PPSV23 a distanza di almeno 8 settimane dall'ultima dose di PCV20.")
            else:
                st.info("👉 Secondo le raccomandazioni attuali, la vaccinazione va completata con almeno 1 dose di PCV20 (preferibile) o PCV15.")
            return True
    return False
...
# --- Funzione di normalizzazione tipo ciclo PCV ---
def normalizza_tipo_ciclo(tipo_dosi):



    # Caso PCV13 puro (solo PCV13)
    if all(v == "PCV13" for v in tipo_dosi):
        return "PCV13"
    # Caso PCV13 presente + altri (serve trattamento specifico)
    elif "PCV13" in tipo_dosi:
        return "ContienePCV13"
    # Ciclo misto solo PCV15/PCV20 (nessun PCV13)
    elif "PCV20" in tipo_dosi and "PCV15" in tipo_dosi:
        return "Misto-PCV20"
    # Solo PCV20
...
def main(data_nascita, eta_mesi, categoria, ha_vaccinazioni, dosi_precedenti):
    ...
    # --- Bambini con eta_mesi < 7 ---
        if categoria == "In buona salute" and eta_mesi < 7:
            ...
            if n_dosi == 1:
                    ...
                    st.info("Completare con altre 3 dosi di PCV20: due nei primi mesi, una a 10-15 mesi. PPSV23 dopo i 24 mesi.")
            elif n_dosi == 2:
    ...
    # --- Funzione di normalizzazione tipo ciclo PCV (da mettere fuori dai blocchi if/elif, una sola volta nel file) ---
    ...
    # --- Bambini con eta_mesi 7-11 ---
            ...
            tipo_ciclo = normalizza_tipo_ciclo(tipo_dosi)
            ...
            elif tipo_ciclo == "PCV20" or tipo_ciclo == "Misto-PCV20":
                if tipo_ciclo == "Misto-PCV20":
                    st.info("ℹ️ Ciclo vaccinale misto PCV15/PCV20: si applicano le regole per PCV20.")

                if n_dosi == 1:
                    st.success("✅ 1 dose già eseguita (con PCV20 presente)")
                    st.info("Completare con 2 dosi: a distanza di 4-8 settimane dall'ultima e fra le dosi.")
                elif n_dosi == 2:
                    st.success("✅ 2 dosi già eseguite (con PCV20 presente)")
                    st.info("Completare con 1 dose a distanza di 4-8 settimane dall'ultima, tra i 10-15 mesi.")
                elif n_dosi == 3:
                    st.success("✅ 3 dosi già eseguite (con PCV20 presente)")
                    st.warning("Verificare: la 4ª dose va eseguita tra i 10-15 mesi.")
                elif n_dosi >= 4:
                    st.success("✅ 4 dosi già eseguite (con PCV20 presente)")
                    st.warning("⚠️ Verifica: la 4ª dose dovrebbe essere tra i 10-15 mesi di età. Se somministrata prima, considerare richiamo.")
                else:
                    st.warning("⚠️ Errore nel numero di dosi. Verificare i dati inseriti.")

            else:
                st.warning("⚠️ Le dosi somministrate non sono omogenee o non riconosciute. Si raccomanda di verificare i dati inseriti.")

    # --- Bambini con eta_mesi 7-11 con patologie a rischio ---
            ...
            if tipo_ciclo == "PCV15":
                if n_dosi == 1:
                    ...
                    st.info("➕ Completare con 2 dosi di PCV20: 1 dose a distanza di almeno 4-8 settimane, poi richiamo tra 10-15 mesi di età. ➕ PPSV23 dopo i 24 mesi di età.")
                elif n_dosi == 2:
                    if all(e < 7 for e in eta_dosi):
                        st.success("✅ 2 dosi di PCV15 già eseguite (entrambe <7 mesi)")
                        st.info("➕ Completare con 2 dosi di PCV20: 1 dose a distanza di almeno 4-8 settimane dalla precedente, poi richiamo tra 10-15 mesi di età. ➕ PPSV23 dopo i 24 mesi di età.")
                    else:
                        st.success("✅ 2 dosi di PCV15 già eseguite (almeno una ≥7 mesi)")
                        st.info("➕ Completare con 1 dose di PCV20 tra 10-15 mesi di età e a distanza di almeno 4 settimane dalla precedente. ➕ PPSV23 dopo i 24 mesi di età.")
                elif n_dosi == 3:
                    st.success("✅ 3 dosi di PCV15 già eseguite")
                    st.info("➕ Somministrare 1 dose di PCV20 tra i 10-15 mesi di età e a distanza di almeno 8 settimane dall'ultima dose. ➕ PPSV23 dopo i 24 mesi di età.")
                else:
                    st.warning("⚠️ Sono state registrate più di 3 dosi di PCV15. Verificare la correttezza dell'inserimento.")
                ...
                if n_dosi == 1:
                    st.success("✅ 1 dose di PCV già eseguita (con PCV20 presente)")
                    st.info("➕ Completare con 2 dosi di PCV20: 1 dose a distanza di almeno 4-8 settimane, poi richiamo tra 10-15 mesi di età. ➕ PPSV23 dopo i 24 mesi di età.")
                elif n_dosi == 2:
                    if all(e < 7 for e in eta_dosi):
                        st.success("✅ 2 dosi di PCV già eseguite (entrambe <7 mesi, con PCV20 presente)")
                        st.info("➕ Completare con 2 dosi di PCV20: 1 dose a distanza di almeno 4-8 settimane dalla precedente, poi richiamo tra 10-15 mesi di età. ➕ PPSV23 dopo i 24 mesi di età.")
                    else:
                        st.success("✅ 2 dosi di PCV già eseguite (almeno una ≥7 mesi, con PCV20 presente)")
                        st.info("➕ Completare con 1 dose di PCV20 tra 10-15 mesi di età e a distanza di almeno 4 settimane dalla precedente. ➕ PPSV23 dopo i 24 mesi di età.")
                elif n_dosi == 3:
                    st.success("✅ 3 dosi di PCV già eseguite (con PCV20 presente)")
                    st.info("➕ Ciclo primario completo. ➕ Raccomandata 1 dose di PPSV23 dopo i 24 mesi di età.")
                elif n_dosi >= 4:
                    st.success("✅ 4 o più dosi di PCV già eseguite (con PCV20 presente)")
                    st.info("➕ Ciclo primario completo. ➕ Raccomandata 1 dose di PPSV23 dopo i 24 mesi di età.")
                else:
                    st.warning("⚠️ Errore nel numero di dosi. Verificare i dati inseriti.")
 ...
 # --- Bambini con eta_mesi 12-23 ---  
            ...
            if gestisci_pcv13(data_nascita, tipo_dosi, categoria, eta_dosi):
                return
            n_dosi = len(dosi_precedenti)
            dosi_sorted = sorted(dosi_precedenti, key=lambda x: x['data'])
            tipo_dosi = [d['vaccino'] for d in dosi_sorted]
